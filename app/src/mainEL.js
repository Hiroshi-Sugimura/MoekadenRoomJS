...
let mainEL = {
	...
	ELreceived: function (rinfo, els, error) {
		if (error) {
			console.error(getNow(), '| mainEL.ELreceived() error:', error);
			console.error(getNow(), '| mainEL.ELreceived() rinfo:', rinfo);
			console.error(getNow(), '| mainEL.ELreceived() els:', els);
			throw error;
		}

		// mainEL.config.EL.debug?console.log( getNow(), '| mainEL.ELreceived() rinfo:', rinfo, '\nels:', els ):0;
