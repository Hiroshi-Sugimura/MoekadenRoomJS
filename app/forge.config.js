 ...
 * @module forge.config
 * @description Electron Forge configuration for MoekadenRoomJS application.
 * Configures build targets, packaging, signing, notarization, and security settings
 * for Windows (Squirrel, APPX), macOS (DMG, ZIP), and Linux (DEB, RPM) platforms.
 * Includes Electron Fuses configuration for enhanced security.
 */
...
export default {
  ...
  packagerConfig: {
    ...
    asar: true,
    // Icon configuration: defaults to src/icons/icon.icns (Mac) and src/icons/icon.ico (Windows) if extensions omitted
    // Adapted to match existing files: using base 'icon' which matches icon.icns. 
    // For Windows, ensure a proper .ico exists or it might fallback/fail.
    icon: './src/icons/icon',
    osxSign: {
  ...
  makers: [
    {
      ...
      config: {
        name: "MoekadenRoomJS",
        target: 'portable',
        // setupIcon: 'src/icons/icon.ico' created from icon256x256.ico
    ...
    },
    {
      ...
      config: {
        applicationId: "Dept.ofHomeElectronicsKAI.MoekadenRoomJS",
        displayName: "MoekadenRoomJS",
        identityName: "Dept.ofHomeElectronicsKAI.MoekadenRoomJS",
        publisher: "CN=C750459E-8B61-41D7-B726-8ED587655544",
        publisherDisplayName: "神奈川工科大学",
        languages: ["JA-JP"],
        assets: "appx/assets",
        // Adapted to match existing asset filename
        Square150x150Logo: "appx/assets/icon.150x150.png",
        makeVersionWinStoreCompatible: "true",
        packageDescription: "MoekadenRoom (The JavaScript version)",
  ...
  hooks: {
    // Note: vcruntime140.dll copy hook omitted as the file was not found in appx/
